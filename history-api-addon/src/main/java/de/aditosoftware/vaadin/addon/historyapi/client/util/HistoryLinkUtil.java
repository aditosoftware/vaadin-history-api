...
/** Represents a utility class which offers some methods which simplify usage with HistoryLinks. */
public class HistoryLinkUtil {
  /**
   * Will handle the given {@link ClickEvent} and check if the event is catchable or not. In
   * addition this will prevent the default action and execute the pushState.
   *
  ...
  public static boolean handleAnchorClick(String uri, ClickEvent event) {
    ...
    if (!isCatchableEvent(event)) return false;
  ...
  /**
   * Will check if the given {@link ClickEvent} is a catchable event. If this returns true if the
   * event can be prevented and the pushState can be executed.
   *
   * @param event The original ClickEvent.
  ...
  public static boolean isCatchableEvent(ClickEvent event) {
    if (event.isMetaKeyDown()
        ...
        || event.isShiftKeyDown()) return false;
